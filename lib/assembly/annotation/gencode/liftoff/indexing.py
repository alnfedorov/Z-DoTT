...
from .annotome import GencodeLiftoffAnnotome, AttrCDS, AttrRNA, AttrGene
from ..indexing import assert_attributes_match
...
def index(assembly_name: str, source_assembly: str, gff3: Path) -> GencodeLiftoffAnnotome:
    logging.info(f"Indexing Liftoff GFF3 for {assembly_name} based on {source_assembly}")
    records = at.preprocess_gff(
        gff3,
        ignore_sources={source_assembly, "cpg", "Eponine"},
        ignore_types={
    ...
    logging.info(f"CDS sources: {sources}")
    ...
    for ind, matches in records["CDS"].items():
        ...
        attrs = AttrCDS(copyind, source, frozenset(parents))
    ...
    logging.info(f"Transcript types: {ttypes}")
    ...
    logging.info(f"Gene types: {gtype}")
    ...
    return at.Annotome(assembly_name, "GENCODE-Liftoff", genes, RNA, cds)
