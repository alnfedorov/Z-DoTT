...
from .annotome import AttrCDS, AttrRNA, AttrGene, GencodeAnnotome
...
def assert_attributes_match(attributes: Iterable[dict[str, str]]):
    ...
    for key, values in unique.items():
        if len(values) > 1:
            logging.warning(f"Inconsistent attributes for {key}: {values}")
            failed = True
        for value, count in values.items():
            if count != elements:
                logging.warning(f"Inconsistent count for {key}={value}: {count}/{elements}")
                failed = True
...
def index(assembly_name: str, gff3: Path) -> GencodeAnnotome:
    logging.info(f"Indexing GENCODE annotation for {assembly_name}...")
    records = at.preprocess_gff(
        gff3,
        ignore_sources={assembly_name, "cpg", "Eponine"},
        ignore_types={
            "five_prime_UTR", "three_prime_UTR", "biological_region", "chromosome", "scaffold", "start_codon",
    ...
    logging.info(f"CDS source types: {sources}")
    ...
    for ind, matches in records["CDS"].items():
        ...
        attrs = AttrCDS(source, frozenset(parents))
    ...
    logging.info(f"Gene biotypes: {gtype}")

    sources = set(x for matches in records["gene"].values() for _, x, _ in matches)
    logging.info(f"Gene sources: {sources}")
    ...
    annotome = at.Annotome(assembly_name, "GENCODE", genes, RNA, cds)
    return annotome
