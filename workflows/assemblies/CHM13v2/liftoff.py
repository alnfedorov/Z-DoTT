...
subprocess.check_call(f'pixi run -e liftoff "{cmd}"', shell=True, cwd=cache)  # Run inside the pixi environment
...
if saveto.name.endswith(".gz"):
    subprocess.check_call(["gzip", polished, "--stdout", ">", saveto], shell=True)
else:
